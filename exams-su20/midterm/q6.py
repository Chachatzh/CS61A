...
def copycat(lst1, lst2):
    ...
    ['c', 'c', 'c']
    """
    def copycat_helper(lst1, lst2, rtn_lst=[]):
        if not lst1 or not lst2:
            return rtn_lst
        if lst2[0] >= 0:
            rtn_lst = rtn_lst.extend([lst[0] for i in range(lst2.pop(0)])
        else:
            rtn_lst = rtn_lst[:lst2.pop(0)]
        return copycat_helper(lst1[1:], lst2, rtn_lst)
